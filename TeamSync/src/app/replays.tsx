...
function canManageReplays(appData: TeamSyncAppData | null) {
...
export default function ReplaysScreen() {
  ...
  const userCanManageReplays = canManageReplays(appData);

  const targetOptions = useMemo<TargetOption[]>(() => {
    const allClubOption: TargetOption = { id: "all-club", label: "Tüm Kulüp" };
    if (appData === null) return [allClubOption];
    return [allClubOption, ...appData.teams.map((team) => ({ id: team.id, label: team.name, teamId: team.id }))];
  }, [appData]);

  const visibleReplays = useMemo(() => {
    if (appData === null) return EMPTY_REPLAYS;

    const roleVisibleReplays = userCanManageReplays
      ? replays
  ...
  }, [activeFilter, appData, replays, userCanManageReplays]);
  ...
  async function handleAddReplay() {
    ...
    const targetUsers = selectedTarget.teamId === undefined ? activeUsers : activeUsers.filter((user) => user.teamIds.includes(selectedTarget.teamId ?? ""));
    ...
    <ScrollView style={styles.scroll} contentContainerStyle={styles.screen}>
      <View style={styles.container}>
        <View style={styles.pageHeader}>
          ...
          <Text style={styles.pageSubtitle}>Maç kayıtları, antrenman videoları ve drill içeriklerini takım bazlı yönet.</Text>
        </View>

        <View style={styles.heroCard}>
          <Text style={styles.heroLabel}>Video kütüphanesi</Text>
          <Text style={styles.heroTitle}>Takım bazlı replay sistemi</Text>
          <Text style={styles.heroSubtitle}>Video linkleri seçili takıma veya tüm kulübe eklenir. Kullanıcılar sadece erişimi olan içerikleri görür.</Text>
        </View>

        {userCanManageReplays ? (
          <View style={styles.topActions}>
            <AppButton title={showCreateForm ? "Form açık" : "Video ekle"} onPress={() => { setShowCreateForm(true); setStatusMessage("Yeni video kaydı bilgilerini doldurabilirsin."); }} disabled={showCreateForm} style={styles.actionButton} />
            <AppButton title="Merkezi datayı yenile" variant="ghost" onPress={loadReplayData} style={styles.actionButton} />
          </View>
        ) : null}

        {showCreateForm && userCanManageReplays ? (
          <View style={styles.section}>
            <View style={styles.sectionHeaderRow}><View style={styles.sectionHeaderText}><Text style={styles.sectionTitle}>Video ekle</Text><Text style={styles.sectionSubtitle}>Video linkini, hedef takımı ve içerik tipini seç.</Text></View><Text style={styles.statusPill}>Yeni</Text></View>
        ...
        <View style={styles.section}>
          <View style={styles.sectionHeaderRow}><View style={styles.sectionHeaderText}><Text style={styles.sectionTitle}>Video listesi</Text><Text style={styles.sectionSubtitle}>{statusMessage}</Text></View><Text style={styles.statusPill}>{visibleReplays.length} görünür</Text></View>
